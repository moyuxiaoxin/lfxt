...
function setday1(tt,pdcs) //主调函数
{
	...
	if(r!=null){
		...
		meizzSetDayzy(r[1],r[2]+1,pdcs);
		
	}
	else{
		outDate="";
		meizzSetDayzy(new Date().getFullYear(), new Date().getMonth() + 1,pdcs);
	}
	dads.display = '';
...
function setdayjs(tt,obj,formatstr) //主调函数
{
	if(null!=formatstr&&formatstr!="")
	formatstring=formatstr;
	if (arguments.length >  3){alert("对不起！传入本控件的参数太多！");return;}
...
function meizzSetDayzy(yy,mm,rr)   //主要的写程序**********
{
  meizzWriteHead(yy,mm);
  //设置当前年月的公共变量为传入值
  meizzTheYear=yy;
    ...
    if (meizzWDay[i]!="")
      { 
		...
		if(i<firstday)		//上个月的部分
		{
			da.innerHTML="<b><font color=#BCBABC>" + meizzWDay[i] + "</font></b>";
			da.title=(mm==1?12:mm-1) +"月" + meizzWDay[i] + "日";
			da.onclick=Function("meizzDayClickzy(this.innerText,-1,"+rr+")");
		...
		else		//本月的部分
		{
			da.innerHTML="<b>" + meizzWDay[i] + "</b>";
			da.title=mm +"月" + meizzWDay[i] + "日";
			da.onclick=Function("meizzDayClickzy(this.innerText,0,"+rr+")");		//给td赋予onclick事件的处理
			//如果是当前选择的日期，则显示亮蓝色的背景；如果是当前日期，则显示暗黄色背景
			if(!outDate)
				da.style.backgroundColor = (yy == new Date().getFullYear() && mm == new Date().getMonth()+1 && meizzWDay[i] == new Date().getDate())?
					"#5CEFA0":"#f5f5f5";
...
function meizzDayClickzy(n,ex,pdcs)  //点击显示框选取日期，主输入函数*************
{
  parent.meizzTheDate=n;
  var yy=meizzTheYear;
  var mm = parseInt(meizzTheMonth)+ex;	//ex表示偏移量，用于选择上个月份和下个月份的日期
  ...
  if (outObject)
  {
	...
	var yjsrq=document.getElementById("yjsrq").value;
	//ksrqDateTemp = ksrq.split(" ");
	//arrksrqDate = ksrqDateTemp[0].split("-");
	arrksrqDate = ksrq.split("-");
	allksrqDate = new Date(arrksrqDate[0], arrksrqDate[1], arrksrqDate[2]);
	
	//yjsrqDateTemp = yjsrq.split(" ");
	//arryjsrqDate = yjsrqDateTemp[0].split("-");
	arryjsrqDate = yjsrq.split("-");
	allyjsrqDate = new Date(arryjsrqDate[0], arryjsrqDate[1], arryjsrqDate[2]);
	alljsrqDate = new Date(yy, mm, n);
	if (alljsrqDate.getTime() < allksrqDate.getTime()) {
	    alert("日期不能小于"+ksrq);
		document.getElementById("jsrq").value=ksrq;
		return false;   
	}
	if(pdcs==1){
		if (alljsrqDate.getTime() > allyjsrqDate.getTime()) {
			alert("日期不能大于"+yjsrq);
			document.getElementById("jsrq").value=yjsrq;
			return false;   
		}
	}
    closeLayer(); 

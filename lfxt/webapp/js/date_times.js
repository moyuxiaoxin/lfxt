...
strFrame+='          onclick="parent.UseTime(this)">';//'     onclick="parent.UseTime(this)">';
...
//var reg = /^(\d+)-(\d{1,2})-(\d{1,2})\s(\d{1,2}):(\d{1,2}):(\d{1,2})/; //含时间
...
if(r!=null){
	...
	if(d.getFullYear()==r[1] && d.getMonth()==r[2] && d.getDate()==r[3])
	{
	   ...
	   parent.meizzTheHour = outObject.value.substring(11,13);
	   parent.meizzTheMinute = outObject.value.substring(14,16);
	   parent.meizzTheSecond = outObject.value.substring(17,19);
	}
	else
	{
	   outDate="";
...
if (mm < 10) {mm = "0" + mm;}
if (hh.length<2&&hh<10) {hh="0" + hh;} //时
if (mi.length<2&&mi<10) {mi="0" + mi;} //分
if (se.length<2&&se<10) {se="0" + se;} //秒

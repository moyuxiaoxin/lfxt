...
strFrame+='<table border=1 cellspacing=0 cellpadding=0 width=142 height=10 bordercolor=#63A3E9 bgcolor=#63A3E9 >';
...
strFrame+='				BORDERCOLORLIGHT=#63A3E9 BORDERCOLORDARK=#FFFFFF width=140 height=10  style="cursor:' + (bMoveable ? 'move':'default') + '">';

strFrame+='</table></td></tr>';
strFrame+='  <tr ><td width=42 height=20 >';
strFrame+='    <table border=1 cellspacing=2 cellpadding=2 BORDERCOLORLIGHT=#63A3E9 BORDERCOLORDARK=#FFFFFF bgcolor=#fff8ec width=140 height=20 >';
var n=0; for (j=0;j<5;j++){ strFrame+= ' <tr align=center >'; for (i=0;i<7;i++){
strFrame+='<td width=20 height=20 id=meizzDay'+n+' style="font-size:12px;display:none;" onclick=parent.meizzDayClick(this.innerText,0)></td>';n++;}
strFrame+='</tr>';}
strFrame+='      <tr align=center >';
for (i=35;i<37;i++)strFrame+='<td width=20 height=20 id=meizzDay'+i+' style="font-size:12px;display:none;"  onclick="parent.meizzDayClick(this.innerText,0)"></td>';
strFrame+='        <td colspan=5 style="color:#1478eb">';
strFrame+='        &nbsp;&nbsp;<span onclick=parent.meizzDayClick(this.innerText,0) style="font-size:12px;cursor: hand"';
strFrame+='         onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" title="当前日期时间">选择</span>&nbsp;&nbsp;</td></tr>';
strFrame+='    </table></td></tr><tr ><td >';
strFrame+='        <table border=0 cellspacing=1 cellpadding=0 width=100%  bgcolor=#FFFFFF height=22 >';
strFrame+='          <tr bgcolor="#63A3E9"><td id=bUseTimeLayer width=55  style="cursor:hand;display:none;" title="点击这里启用/禁用时间"';
...
strFrame+='             <td style="cursor:hand;display:none;" onclick="parent.tmpSelectHourInnerHTML(this.innerText.length==3?this.innerText.substring(0,1):this.innerText.substring(0,2))"';
...
strFrame+='             <td style="cursor:hand;display:none;" onclick="parent.tmpSelectMinuteInnerHTML(this.innerText.length==3?this.innerText.substring(0,1):this.innerText.substring(0,2))"';
strFrame+=' onmouseover="style.backgroundColor=\'#aaccf3\'" onmouseout="style.backgroundColor=\'#63A3E9\'"';
strFrame+=' title="点击这里选择时间" align=center width=50>' ;
...
function setday(tt,obj,formatstr) //主调函数
{
...
function meizzToday()  //Today Button
  {
    ...
    if(outObject){
		if (bUseTime)
		{
			outObject.value= parent.meizzTheYear + "年" + format( parent.meizzTheMonth) + "月" + 
							format(parent.meizzTheDate) + "日" + format(parent.meizzTheHour) + ":" + 
		...
		{
			outObject.value= parent.meizzTheYear + "年" + format( parent.meizzTheMonth) + "月" + 
							format(parent.meizzTheDate)+"日"; //注：在这里你可以输出改成你想要的格式
		}
    }
    closeLayer();
  }
...
function WriteDateTo(yy,mm,n,hh,mi)
{
	...
	else
	{
		...
		{
			outObject.value= yy + "-" + format(mm);			
		}else if(formatstring=="china")
		{
			outObject.value= yy + "年" + format(mm) + "月" ; //注：在这里你可以输出改成你想要的格式
		}else if(formatstring=="1")
		{
			outObject.value= yy + "年" + format(mm) + "月" + format(n) + "日"; //注：在这里你可以输出改成你想要的格式
		}
		else
		{
...
function WriteDateTo(yy,mm)
{
	if (bUseTime)
	{
	    outObject.value= yy + "-" + format(mm); //注：在这里你可以输出改成你想要的格式
	}
	else
	{
		if(formatstring=="yyyy-mm")
